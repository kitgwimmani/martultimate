      ...
      <Link to='/category/createCategory' className='btn btn-success'>Add +</Link>
            <Form>
              <InputGroup className='my-3' style={{ width: '100%'}}>
                <Form.Control onChange={(e) => setSearch(e.target.value)} placeholder='Search Category' />
              </InputGroup>
            ...
            <tbody>
              {
                category.filter((data) => {
                  ...
                  <tr key={i}>
                    ...
                    <td> <ButtonGroup>
                      <Link to={`updateCategory/${data.id}`} className='btn btn-warning btn-sm'>Update</Link>
                      <Dropdown >
